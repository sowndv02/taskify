...
namespace taskify_api.Controllers
{
    ...
    public class StatusController : ControllerBase
    {
        ...
        [HttpGet("color/{id:int}", Name = "GetStatusByColorId")]
        public async Task<ActionResult<APIResponse>> GetByColorIdAsync(int id)
        {
            try
            {
                ...
                List<Status> model = await _statusRepository.GetAllAsync(x => x.ColorId == id);
                _response.Result = _mapper.Map<List<StatusDTO>>(model);
                return Ok(_response);
            }
            catch (Exception ex)
            {
